<?php
	...
	WHERE reservas_rel_quartos.CODRESERVA='{$codreserva->CODRESERVA}'
	AND reservas_rel_tipo_quarto.CODRESERVA='{$codreserva->CODRESERVA}'
	...
	if(mysql_num_rows($acomodacoes) != 0)
	{
		...
		while( $acomodacao = mysql_fetch_object($acomodacoes))
		{
			...
			AND reservas.CODRESERVA='{$codreserva->CODRESERVA}'
				...
				switch($guest->FAIXA_ETARIA)
{
	...
	INNER JOIN reservas_rel_guest ON reservas_rel_guest.CODGUEST=guest.CODGUEST
	INNER JOIN reservas ON reservas_rel_guest.CODRESERVA=reservas.CODRESERVA
	WHERE reservas.CODRESERVA='{$codreserva->CODRESERVA}'
	ORDER BY FAIXA_ETARIA ASC");
	...
	$tipo_quarto = mysql_query("SELECT quartos_tipo.NOME FROM quartos_tipo 
	INNER JOIN reservas_rel_tipo_quarto ON reservas_rel_tipo_quarto.CODTIPOQUARTO=quartos_tipo.CODQUARTOTIPO
	WHERE reservas_rel_tipo_quarto.CODRESERVA='{$codreserva->CODRESERVA}'");
	
	if( mysql_num_rows($tipo_quarto)){
		$tipo_quarto = mysql_fetch_object($tipo_quarto);
	} 
		...
		foreach($pieces as $pce)
		{
			...
			switch($i)
			{
				...
				case 8:
					...
					$piece .= $t;
					$piece .= "<br/>";			
					$piece .= "<div>".utf8_encode(getLabel('LABEL_CRIANCAS_5ANOS', $_SESSION['LANGUAGE'])).": {$soma_crianca_5}</div>";
					$piece .= "<div>".utf8_encode(getLabel('LABEL_CRIANCAS_6A12', $_SESSION['LANGUAGE'])).": {$soma_crianca_6_12}</div>";
					$piece .= "<div>".utf8_encode(getLabel('LABEL_CRIANCAS_ACIMA12', $_SESSION['LANGUAGE'])).": {$soma_crianca_12}</div>";
				...
				case 10:
					//$piece .= $compl;
					$piece .= $tipo_quarto->NOME;
				break;
				case 11:
					if($reserva->FORMA_PGTO == "1")
					{
						$piece .= "<br/>".utf8_encode(getLabel('LABEL_DEPOSITO', $_SESSION['LANGUAGE']));
					}
					elseif($_POST["FORMA_PGTO"] == "2")
					{
						$piece .= "<br/>".utf8_encode(getLabel('LABEL_CARTAO_CREDITO', $_SESSION['LANGUAGE']));
					}
				break;
				case 12:
