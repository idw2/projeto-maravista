<?php
	...
	if( $_POST["screen_option_2_ok"] == "NOACTION")
	{
		...
		$dormitorio_3 = new Reserva2(
			...
			$_POST["select_quarto_tipo_2"]
			);
		
		$tqts->VALOR = $dormitorio_3->total;
		...
		for( $i=0; $i<(int)$_POST["screen_pessoa_2"]; $i++)
		{
			$html .= "<div class='' style=''>";
				...
				$guid_text = "quarto_3_text_name_".$i;
				$guid_select = "quarto_3_select_".$i;
		...
		//$valor_3 = ($dias * $tqts->VALOR);
		$valor_3 = $tqts->VALOR;
		$html .= $dormitorio_3->getTextos();
		/*
		if( $dias == 1 )
		{
		...
		{
			$html .= "<div class='LabelAcomodacaoTitle left'>R$ ".formataReais($tqts->VALOR)." x $dias ".getLabel('LABEL_DIAS', $_SESSION['LANGUAGE'])."= ".formataReais($valor_3)."</span></div>";
		}
		*/
		$bruto = ($valor_3+$bruto);
		...
		if( $fazer_soma > 2)
		{
			...
			if( $soma_adulto > 2 )
			{
				...
				( $dias == 1 ) ? $valor_pacote = $tqts->VALOR : $valor_pacote = $valor_3;
			...
			if( $soma_crianca_5 > 1 )
			{
				...
				$valor_3 = ($soma_crianca_5+$valor_3);
			...
			if( $soma_crianca_6_12 != 0 )
			{
				...
				$valor_3 = ($soma_crianca_6_12+$valor_3);
			...
			if( $soma_crianca_12 != 0 )
			{
				...
				$valor_3 = ($soma_crianca_12+$valor_3);
